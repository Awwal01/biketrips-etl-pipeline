...
SPARK_JOB_DRIVER = {
    "sparkSubmit": {
        ...
        # "entryPointArguments":  # <s3_input_path> <s3_output_path> <dag_name> <task_id> <correlation_id>
        "entryPointArguments": [f"s3://{S3_BUCKET_NAME}/test/data/transformed/green/", f"s3://{S3_BUCKET_NAME}/test/output_dag",
        dag_name, emr_serverless_job_task_id , correlation_id], 
...
SPARK_CONFIGURATION_OVERRIDES = {
    "monitoringConfiguration": {"s3MonitoringConfiguration": {"logUri": f"s3://{S3_BUCKET_NAME}/logs"}}
    }

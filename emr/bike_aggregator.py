...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder.appName("BikeData").getOrCreate()
    ...
    subset_df = df.select(
        "starttime", "start_station_name", "tripduration", "trip_distance"
    )
    ...
    wind_date_df = (
        subset_df.withColumnRenamed("tripduration", "trip_duration")
        .withColumn("measurement_year", F.year(subset_df.starttime))
        .select(
            "start_station_name", "measurement_year", "trip_duration", "trip_distance"
        )
    )
    ...
    agg_wind_df = wind_date_df.groupBy("start_station_name", "measurement_year").agg(
        F.min(wind_date_df.trip_duration).alias("min_trip_duration"),
